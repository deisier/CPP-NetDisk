...
//    setsockopt(m_sock, SOL_SOCKET, SO_SNDTIMEO, (char *)&nNetTimeout, sizeof(DWORD));
    ...
    //禁用 TCP-NODELAY(作用：该选项会禁用Nagle算法（减少小包的数量）。)
    int value = 1;
...
*/
    ...
    int DataLen = nlen + 4;
    //用vector 也会创建堆区的数组，然后再执行析构的时候就自动回收内存了（这个函数执行结束自动回收了就）
    std::vector<char> vecbuf;
    vecbuf.resize( DataLen );

 //   char* buf = new char[ DataLen ]; 用这个还得要用delete去回收
    char* buf = &*vecbuf.begin(); //调用begin()函数返回堆区的开始地址
    char* tmp = buf;
    *(int*) tmp = nlen;
    tmp+= sizeof(int);
